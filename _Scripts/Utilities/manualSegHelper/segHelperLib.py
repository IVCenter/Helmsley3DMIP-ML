...
def createTrainingData (imageDicomPath:str=sampled_dicom_folder_path, maskDicomPath:str=sampled_label_folder_path):
    ...
    mask_dcms = [pydicom.read_file(maskDicomPath + '/' + f) \
                  for f in listdir(maskDicomPath) if isfile(join(maskDicomPath,f)) if f.endswith(".dcm")]
    mask_dcms.sort(key = lambda x: int(x[0x20, 0x32][1]))
    
    if(len(image_dcms) != len(mask_dcms)):
    ...
    for image, mask in zip(image_dcms, mask_dcms):
